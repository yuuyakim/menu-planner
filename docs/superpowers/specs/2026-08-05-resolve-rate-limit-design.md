...
この超過を絞るのは `max-instances=2` ではなく、**同時に処理中のリクエスト数**。
Cloud Run の `--concurrency` は既定80（1インスタンスあたり）で、このサービスでは
ピン留めしていない。2インスタンスなら理論上 約160件が Check を通り抜けてから
最初の Record が効き始める。全体上限300に対して最悪 300+160=460回、
金額にして約320円/日。これは受け入れる（§7.1）。
...
| 全体 | 300回/日 | 名目 約210円/日（約6,300円/月）、典型的にはその半分。非ログイン100人が1日3回使っても届かない。ただし Check-then-Record の競合（§5）を含めると最悪 約320円/日（約9,600円/月）。`--concurrency` を既定80のまま運用しているため |

Cloud Run の `--concurrency` をピン留め（例: 10〜20）すれば、この上振れ幅は
インスタンスあたりの同時実行数に比例して縮む。現状は未設定＝既定80のまま。
