...
現在の献立マスタ120件は想定利用者を決めずに作っている。**これは機能追加である前に
...
`elaborate` 40件（マスタ120件の1/3）を実際に見ると、3つが混ざっている:
...
**難易度は3値×各40件と綺麗に設計されているが、それは「手間」の軸として作られたもの。**
日常性はそれと直交する別の軸で、`elaborate` の中で3つに割れている。
`easy` / `normal` の80件は日常寄りのはずだが、付与時に一度通しで確認すること。
...
選択肢:
- 商用利用可の素材（写真AC等）から120件分を手作業で紐付ける。ライセンス条件の確認が要る
