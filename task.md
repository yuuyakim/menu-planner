...
- [ ] 🔴 テスト: 同じ食材が複数の献立に出たら1件にまとまり、`usedIn` に両方が入る
- [ ] 🔴 テスト: 調味料は登録されていないので、リストは買う食材だけになる
- [ ] 🔴 テスト: 並び順（カテゴリ順 → カナ順）
- [ ] 🔴 テスト: 重複した献立IDは1件として扱う
- [ ] 🔴 テスト: 0件・8件以上は不正（`ErrInvalidMenuIDs`）
- [ ] 🔴 テスト: 存在しない献立IDが混ざれば `ErrMenuNotFound`
- [ ] 🟢 `service.ShoppingList` を実装
