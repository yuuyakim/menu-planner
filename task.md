...
- [ ] 🔴 テスト: 200 と3件
- [ ] 🔴 テスト: 存在しない献立で 404
- [ ] 🔴 テスト: gateway 障害で 502（`gateway.ErrSearchFailed` → 502）
- [ ] 🔴 テスト: 3件未満でも200
- [ ] 🟢 `MenuHandler.Recipes` を実装
- [ ] 🔧 3-D の最悪9.6秒に対し、全体の締め切りを設けるか判断する
- [ ] 🔧 実機確認: stub で3件返ること
- [ ] 🔧 実機確認: **Brave の実キーで本物のレシピが3件返ること**（※要APIキー）
- [ ] 🔧 実機確認: **日本語のレシピが返ること**。返らなければ `country` / `search_lang` を付ける
      → 3-C では意図的に省いた。APIリファレンスが値を列挙しておらず（"AR AU AT +35 more"）、
        日本の値を推測で入れると 422 になる恐れがあり、キー無しでは検証できないため。
...
- [ ] 🔧 マイグレーション `000002_create_recipe_link_caches`（menu_id PK / links jsonb / fetched_at）
- [ ] 🔴 テスト: 初回は gateway を呼び、2回目は呼ばない
- [ ] 🔴 テスト: TTL 7日を過ぎたら再取得する（境界値: 7日ちょうどはヒット）
- [ ] 🔴 テスト: キャッシュが壊れていても gateway にフォールバックする
- [ ] 🔴 テスト: gateway が0件を返した場合もキャッシュする（毎回叩き直さない）
- [ ] 🔴 テスト: gateway 障害時はキャッシュを書かない
- [ ] 🟢 キャッシュを実装（`RecipeSearchGateway` をキャッシュで包む形にするか、
      service に持たせるかは実装時に判断する）
