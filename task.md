...
- [ ] 🔴 テスト: カテゴリ→カナの順で全件返る
- [ ] 🔴 テスト: 未認証でも取得できる
- [ ] 🟢 `GET /ingredients`（repository に全件取得を追加）
...
- [ ] 🔴 テスト: 手持ちを含む献立が返り、`matched` / `missing` が正しい
- [ ] 🔴 テスト: 並び順（不足の少ない順 → 一致の多い順 → カナ順）
- [ ] 🔴 テスト: 手持ちと重ならない献立は返らない
- [ ] 🔴 テスト: 0件指定は 400／重複IDは1件として扱う／存在しないIDは 404
- [ ] 🔴 テスト: 上位20件で打ち切る
- [ ] 🟢 `POST /menus/search-by-ingredients`

#### 13-D: 食材を選ぶUI `feat/ui-ingredient-picker`

- [ ] 🔴 テスト: カテゴリごとに分かれて並ぶ／選択・解除できる
- [ ] 🔴 テスト: 1つも選んでいなければ検索できない
- [ ] 🟢 食材ピッカー
