...
- [ ] 🔴 テスト: `GET /menus/:id/ingredients` が200で食材を返す／不正UUIDで400／存在しないIDで404
- [ ] 🔴 テスト: `POST /shopping-list` が200／0件・8件以上で400／存在しないIDで404
- [ ] 🟢 ハンドラを実装し `/api/v1` に結線（検索系と同じレート制限）
- [ ] 🔧 `api/openapi.yaml` を更新し、型を再生成（`make gen-api`）
- [ ] 🔧 契約テストに追加
