...
`menus` に入手性か価格帯の列を足して軸を分ける。値の刻みは要検討
（「スーパーで買える / 買えない」の2値で足りるか、価格帯まで要るか）。
A のモードと組み合わせて既定の絞り込みを決める。

> 実際の価格を持つ話は H。ここは**静的な分類**に留める想定。
...
F（保存）と組み合わせて「引き直す→動かす→固める→保存する」という流れになる。
**F と一緒に設計すると筋が良い。**
...
**着手するなら、まず C（価格帯の静的分類）で「安い献立に寄せる」までを実装し、
実額の算出はやらない**のが現実的。レビューの動機（使いすぎたくない）は
それでかなり満たせる。実額をやるなら、分量データの整備が前提。
...
```
A (ターゲット層・モード) ─┬─→ C (入手性・価格帯)
                          ├─→ D (季節)
                          └─→ H (予算) ※C の先。分量データも要る

B (メイン/副菜)  … 2.1 の「1食＝主菜1品」前提に波及。影響範囲が最大

F (週間献立の保存) ─→ G (日付の並べ替え)   … 依存が浅い。単独で着手できる

E (画像)  … 素材の調達方針を決めるまで着手できない
```

**着手順の提案: F+G → A → C → D → B → E → H。**
