...
- 食材はカテゴリ（野菜・肉・魚介・卵乳・主食・調味料・その他）ごとに分けて並べる
- **常備品として扱う調味料（醤油・塩・砂糖など）は買うものと分けて表示する**。
  毎回リストに載せても大半は既に持っており、本当に買うものが埋もれるため
...
| category | text | NOT NULL、CHECK（vegetable / meat / seafood / dairy_egg / staple / seasoning / other） |
| is_pantry_staple | boolean | NOT NULL DEFAULT false |
| created_at | timestamptz | NOT NULL DEFAULT now() |

`is_pantry_staple` は「多くの家庭に常備されている」ことを表す（醤油・塩・砂糖・油など）。
買い物リストで買うものと分けて出すために使う。カテゴリが `seasoning` でも
常備しないもの（例：カレールー）はあるため、カテゴリとは別の列にする。
...
```json
{
  ...
  "pantryItems": [
    { "id": "...", "name": "醤油", "nameKana": "しょうゆ", "category": "seasoning" }
  ]
}
```
...
- `pantryItems` は常備品として扱う調味料。買うものと混ぜない
- 並び順はカテゴリ（野菜→肉→魚介→卵乳→主食→調味料→その他）、同カテゴリ内は名前のカナ順
