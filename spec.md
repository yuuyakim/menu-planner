...
>
> **実装するときはいずれも free に入れる。** プレミアムは「1週間まとめて計画する」という
> 深さに対する課金であり（2.11、`2026-07-24-weekly-premium-retier-design.md` §3.1・§4）、
> 利用者が「困る」と言ったことの解消それ自体を有料化しない。
>
...
- **premium 限定。要ログイン**（2.11）。1食分の検索（2.1）は free のまま変わらない
...
> **未ログインは401、ログイン済みのfreeは403。** free/未ログインが週間画面を開いたときは、
> 生成UIの代わりに「プレミアムなら1週間まとめて計画できる」というロック付きプレビューと
> ログイン/アップグレード導線を出す（存在は隠さない）。単発（2.1）の画面・挙動は変えない。
> 詳細は 2.11 と `2026-07-24-weekly-premium-retier-design.md`（§3.2・§3.3・§7）。
...
- 未認証でも1食分の献立検索（2.1）は利用できる。**週間献立（2.2）は要ログイン＋premium**
  （2.11）で、履歴・お気に入りと同様に未認証では使えない
...
- **買い物リストの生成そのもの（`POST /shopping-list`）は未認証でも使える。**
  献立IDの配列から組み立てるステートレスなAPIで、週間献立の生成が premium 限定（2.2）に
  なったあとも変わらない（件数で free を縛らないため。設計 §3.5）
...
- **premium 限定。要ログイン**（2.11）。保存・一覧・削除のいずれも free / 未ログインでは
  使えない（未ログイン401 / free403）。週間献立の作成自体（2.2）が premium 限定になった
  ため、保存だけを未認証・freeに開ける理由も無くなっている
- 保存できるのは**7日分をひとまとまり**として。個々の献立の保存はお気に入り（2.6）が担う
- 保存した週を**開く**と、その週が作成直後と同じ状態に戻る。
  引き直し（2.2）も買い物リスト（2.7）もそのまま続けられる
- 保存一覧から個別に削除できる
- **保存の上限は premium 50件**（2.11）。超過した状態での保存は 409 を返し、
  古いものを消すよう促す。件数は 409 の `detail` にサーバが組み立てて入れる（フロントは持たない）
- **買い物リストの取得・永続化も premium 限定**（2.11）。保存済みの週に限り、
...
> **grandfather しない。** 週間まわりが premium 限定になる前に free で保存した週間献立が
> あっても、データは削除しない（3.4項）が free の間は一覧が 403 で見えなくなる。
> upgrade すれば再び見える。旧データだけを free に見せる特別扱いは入れない
> （認可の分岐が増え、実装と説明が複雑になるため）。詳細は
...
> premium 限定であり、free はどちらも 403 になる（2.11）。
...
- 1ユーザーあたりの上限は premium 50件（free は保存自体が premium 限定のため到達しない。
  2.8 / 2.11）。上限判定はアプリ側で行う
...
| POST | `/menus/suggest-weekly` | **必須＋premium** | 7日分を提案（2.2 / 2.11。未ログイン401 / free403） |
| POST | `/menus/reroll-day` | **必須＋premium** | 週間献立の1日だけを引き直す（2.2 / 2.11。未ログイン401 / free403） |
...
| Method | Path | 認証 | 説明 |
| --- | --- | --- | --- |
| GET | `/weekly-menus` | **必須＋premium** | 保存した週の一覧（新しい順、中身の7件を含む）。未ログイン401 / free403 |
| POST | `/weekly-menus` | **必須＋premium** | 保存（body: `{"days": [{"day": 1, "menuId": "..."}, ...]}`）。未ログイン401 / free403 |
| DELETE | `/weekly-menus/:id` | **必須＋premium** | 保存した週を1件削除。未ログイン401 / free403 |
| GET | `/weekly-menus/:id/shopping-list` | **必須＋premium** | 保存済みの週の買い物リストを取得（2.11）。未ログイン401 / free403 |
| PUT | `/weekly-menus/:id/shopping-list` | **必須＋premium** | 買い物リストの差分を一括置換（2.11）。未ログイン401 / free403 |

週間まわり一式が premium 限定になった（2.2 / 2.8 / 2.11、`2026-07-24-weekly-premium-retier-design.md`）。

- `POST` は 7日分ちょうどを要求する。過不足は 400
- 保存上限（premium 50件）に達している状態の `POST` は **409**（2.8 / 2.11）
