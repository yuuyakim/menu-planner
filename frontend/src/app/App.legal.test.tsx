...
// 法務3ページは未ログインでも見えて欲しい（RequireAuth で包まない）。
// テストは既定のハンドラ（未ログイン）のまま実行し、リダイレクトされず
// 各ページの見出しが出ることを確認する。
// アサートは安定部分文字列のみ。「（草案）」などは Task 4 で整形されるため使わない。
describe('法務ページ（未ログイン）', () => {
  it('/legal/tokushoho で特定商取引法に基づく表記を表示する', () => {
    renderWithProviders(<App />, { route: '/legal/tokushoho' })

    expect(
      screen.getByRole('heading', { name: /特定商取引法/ }),
    ).toBeVisible()
    expect(
