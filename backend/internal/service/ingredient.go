...
var (
	...
	// ErrInvalidMatchSort は並び順の指定が MatchSort の2値のどちらでもない
	// ことを表す（400）。
	//
	// 既定に丸めない。利用者の指定を黙って読み替えると、違う条件で検索した
	// 結果を正しい答えとして返すことになる。
	//
	// 判定するのは handler（値をここまで持ち込ませない）だが、エラーは
	// service に置く。並び順が何であるかを決めているのが MatchSort であり、
	// handler 固有のエラーにすると「不正な値」の定義が2か所に散る。
	ErrInvalidMatchSort = errors.New("並び順の指定が不正です")
)
